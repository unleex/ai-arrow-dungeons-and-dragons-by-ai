...
@rt.message(StateFilter(FSMStates.get_number_of_players))
async def counting_players(msg: Message, state: FSMStates):
...
@rt.message(StateFilter(FSMStates.DnD_init_players))
async def get_descriptions(msg: Message, state: FSMStates, chat_data: dict):
    ctx = await state.get_data()
